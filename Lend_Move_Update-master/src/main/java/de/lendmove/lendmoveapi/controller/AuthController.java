...
public class AuthController {
  ...
  //  private JwtTokenValidator tokenValidator;
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        System.out.println("am resgistering user-------" + signUpRequest);
 ...
 */
    ...
    public ResponseEntity<?> updateUser(@RequestBody SignupRequest user) {
        System.out.println("updating user data" + user);
       User updatedUser = new User();
        if(user.getId()!=null && userRepository.existsById(user.getId()))
        {
            updatedUser = userRepository.findById(user.getId()).get();
                    ...
                    {
                        /*
                        if(user.getPassword()!=null && user.getCheckpassword()!= null && !user.getPassword().equals(user.getCheckpassword()))
                        {
                            return ResponseEntity.ok(new MessageResponse("Die Passwörter stimmen nicht !"));
                        }

                         */
                        User usernew =  modifiedUserValue(user, updatedUser);

                        System.out.println("www " +  usernew.getPassword());
        //                System.out.println("ddede "+ usernew.getCheckpassword());
 ...
 */
    ...
    @GetMapping("/lendmove/api/auth/")
    public ResponseEntity<?> searchUserByUsername(@RequestBody LoginRequest user)
    {
        Map<String, Object> response = new HashMap<>();
        ...
        if(user.getUsername()!=null && userRepository.existsByUsername(user.getUsername()))
        {
         ...
         //   return ResponseEntity.ok(new MessageResponse(theUser.getUsername() + theUser.getId()));
            return new ResponseEntity<>(response, HttpStatus.OK);
            ...
            return ResponseEntity
                    .badRequest()
                    .body(new MessageResponse("The User does not exit"));
    ...
    public ResponseEntity<?> searchUserByUsername(@RequestParam("username") String username)
    {
        ...
        {
            theUser = userRepository.findByUsername(username).get();
            response.put("User", theUser);
            System.out.println("the name ist :  "+ theUser.getUsername());
            return new ResponseEntity<>(response, HttpStatus.OK);
        }else {
            return ResponseEntity
                    .badRequest()
